...
app.post('/api/generate-business', async (req, res) => {
  try {
    ...
    const dateStr = new Date().toISOString().split('T')[0]; // YYYY-MM-DD
    const limitKey = `gemini_business_gen_${userId}_${dateStr}`;
  ...
  try {
    ...
    for (let i = 0; i < leadIds.length; i += 50) {
      ...
      const { data: chunkLeads, error: chunkErr } = await supabaseAdmin
        ...
        .not('email', 'eq', '')
        .not('email', 'is', null)
        .limit(5 - leads.length);
        
      if (chunkErr) leadsErr = chunkErr;
      if (chunkLeads) leads.push(...chunkLeads);
    ...
    const onResult = async (lead) => {
      ...
      if (!researchSummary || researchSummary.length < 20) {
        try {
          ...
          if (researchSummary && (researchSummary.includes('## ⚡ Personalised Detail\nwork') || researchSummary.includes('Research data could not be parsed') || researchSummary.includes('No specific details found.'))) {
            log(`[Filter] Dropped ${lead.company || 'lead'}: AI Deep Research returned blank/fallback data.`);
            return;
          }
