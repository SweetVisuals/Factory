...
const CampaignEmails = ({ campaignId }: Props) => {
  ...
  if (isLoading) {
    return (
      <div className="animate-pulse space-y-3">
        <div className="h-8 bg-foreground/[0.02] rounded-xl w-full"></div>
        <div className="h-8 bg-foreground/[0.02] rounded-xl w-full"></div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      {/* Compact Mini-Header */}
      <div className="flex justify-between items-center bg-foreground/[0.02] p-3 rounded-xl">
        ...
        <Button
          ...
          variant="ghost"
          className="h-7 px-3 bg-primary text-primary-foreground hover:bg-primary/90 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all flex items-center gap-1 border-none"
        >
          <Plus size={10} />
          Link Accounts
        </Button>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[480px] bg-[#09090e] border-none shadow-2xl rounded-3xl p-8 animate-in zoom-in-95 duration-200 text-foreground">
            ...
            {availableAccounts.length === 0 ? (
              ...
              availableAccounts.map(account => {
                ...
                return (
                  <div 
                    key={account.id} 
                    className={`flex items-center space-x-4 p-4 rounded-xl transition-all cursor-pointer group ${
                      isSelected
                  ...
                  >
                    <Checkbox
                      ...
                      checked={isSelected}
                      className="border-foreground/10 data-[state=checked]:bg-foreground data-[state=checked]:text-background w-5 h-5 rounded-md"
                    />
          ...
          <div className="pt-6">
            <Button
              ...
              className="w-full h-12 bg-primary text-primary-foreground hover:bg-primary/95 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-primary/10 transition-all border-none"
            >
      ...
      <div className="space-y-1.5 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
        {emailAccounts.length === 0 ? (
          <div className="py-12 flex flex-col items-center justify-center bg-foreground/[0.01] rounded-2xl space-y-4">
            <div className="p-4 bg-foreground/[0.02] rounded-xl text-foreground/20">
          ...
          emailAccounts.map((account) => (
            <div key={account.id} className="p-3 bg-foreground/[0.02] hover:bg-foreground/[0.04] rounded-xl transition-all group flex items-center justify-between">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-2 text-[8px] font-black uppercase tracking-widest mt-0.5 text-foreground/40">
                    <span className="bg-foreground/[0.05] px-1.5 py-0.5 rounded-md">{account.provider || 'SMTP'}</span>
