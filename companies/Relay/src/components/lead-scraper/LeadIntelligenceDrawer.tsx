...
export const LeadIntelligenceDrawer: React.FC<LeadIntelligenceDrawerProps> = ({ lead, open, onClose, onReResearch, onLeadUpdate }) => {
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className={`fixed top-12 bottom-0 left-0 right-0 z-[90] h-[calc(100vh-3rem)] bg-black/60 backdrop-blur-sm transition-opacity duration-300 ${open ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        onClick={onClose}
      />

      {/* Drawer */}
      <div className={`fixed top-12 right-0 z-[90] h-[calc(100vh-3rem)] w-full max-w-[780px] bg-[#0a0a0a] border-l border-white/10 shadow-[0_0_80px_-20px_rgba(0,0,0,0.8)] transition-transform duration-300 ease-out ${open ? 'translate-x-0' : 'translate-x-full'} flex flex-col`}>
