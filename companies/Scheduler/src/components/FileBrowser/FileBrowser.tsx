...
export function FileBrowser({
  ...
  const [accountSettings, setAccountSettings] = useState<Record<string, { theme: string; goal: string }>>({});
  ...
  useEffect(() => {
    const loadProfiles = async () => {
      ...
      try {
        ...
        const { data: settingsData } = await supabase.from('tiktok_account_settings').select('*');
        if (settingsData) {
          const settingsMap: Record<string, { theme: string; goal: string }> = {};
          settingsData.forEach(s => {
            settingsMap[s.account_id] = { theme: s.theme || '', goal: s.goal || '' };
          });
  ...
  const handleSaveSettings = async (accountId: string) => {
    setSavingSettingsId(accountId);
    try {
      const setting = accountSettings[accountId] || { theme: '', goal: '' };
      
      const { data: existing } = await supabase
        .from('tiktok_account_settings')
        .select('id')
      ...
      if (existing) {
        await supabase
          .from('tiktok_account_settings')
          .update({ theme: setting.theme, goal: setting.goal, updated_at: new Date().toISOString() })
          .eq('account_id', accountId);
      } else {
        await supabase
          .from('tiktok_account_settings')
          .insert({ account_id: accountId, theme: setting.theme, goal: setting.goal });
      }
                  ...
                  return (
                    <div key={profile.id} className="bg-black/40 border-transparent rounded-sm overflow-hidden flex flex-col h-full">
                      ...
                      <div className="p-4 border-b border-white/10 bg-black/20 flex flex-col gap-3">
                        <div>
                          <label className="text-[10px] uppercase tracking-wider text-white/50 mb-1 block">Theme / Aesthetic</label>
                          <Input 
                            value={accountSettings[profile.id]?.theme || ''} 
                            onChange={(e) => setAccountSettings(prev => ({...prev, [profile.id]: { ...prev[profile.id], theme: e.target.value }}))}
                            placeholder="e.g. Black luxury city" 
                            className="h-8 bg-white/5 border-white/10 text-sm"
                          />
                        </div>
                        <div>
                          <label className="text-[10px] uppercase tracking-wider text-white/50 mb-1 block">Goal</label>
                          <Input 
                            value={accountSettings[profile.id]?.goal || ''} 
                            onChange={(e) => setAccountSettings(prev => ({...prev, [profile.id]: { ...prev[profile.id], goal: e.target.value }}))}
                            placeholder="e.g. Promote Mani Rae track" 
                            className="h-8 bg-white/5 border-white/10 text-sm"
                          />
                        </div>
                        <Button 
                          size="sm" 
                          variant="secondary"
                          className="w-full h-8 bg-primary/20 hover:bg-primary/30 text-primary border-none"
                          onClick={() => handleSaveSettings(profile.id)}
                          disabled={savingSettingsId === profile.id}
                        >
                          {savingSettingsId === profile.id ? <RefreshCw className="w-3 h-3 animate-spin mr-2" /> : <CheckSquare className="w-3 h-3 mr-2" />}
                          Save Settings
                        </Button>
                      </div>
