...
import { ChevronDown, ChevronUp, Clock, FileText, Hash, Layout, Loader2, Trash2, UploadCloud, X, Zap } from 'lucide-react';
...
export const QuickModeDialog: React.FC<QuickModeDialogProps> = ({ isOpen, onClose, folderId, folderName, accountId, userId }) => {
    ...
    // Auto-save changes to the currently selected template
    useEffect(() => {
        if (!selectedTemplateId || isProcessing || !isOpen) return;
    ...
    }, [hashtags, selectedAspectRatio, customTitle, slideCountOverride, textReplacements, selectedTemplateId]);
    ...
    const saveCurrentSettings = async (targetTemplateId?: string) => {
        const templateToSave = templates.find(t => t.id === (targetTemplateId || selectedTemplateId));
        if (!templateToSave || !userId) return null;

        const currentSettings = {
            name: customTitle || templateToSave.name,
            hashtags,
            aspectRatio: selectedAspectRatio,
            slideCount: slideCountOverride,
            textOverlays: templateToSave.textOverlays.map(overlay => {
        ...
        // Check if anything actually changed from the template's own perspective
        const isDifferent = 
            currentSettings.name !== templateToSave.name ||
            currentSettings.slideCount !== templateToSave.slideCount ||
            currentSettings.aspectRatio !== templateToSave.aspectRatio ||
            JSON.stringify([...currentSettings.hashtags].sort()) !== JSON.stringify([...(templateToSave.hashtags || [])].sort()) ||
            JSON.stringify(currentSettings.textOverlays.map(o => o.text)) !== JSON.stringify(templateToSave.textOverlays.map(o => o.text));

        if (!isDifferent) {
            console.log('🔍 QuickMode: No changes detected for template', templateToSave.id);
            return templateToSave;
        }

        // Check if an EXACT MATCH of these settings already exists in another template (to avoid duplicates)
        const exactMatch = templates.find(t => {
            if (t.id === templateToSave.id) return false; // Skip itself
            const nameMatches = t.name === currentSettings.name;
            const settingsMatch = 
                t.slideCount === currentSettings.slideCount &&
                t.aspectRatio === currentSettings.aspectRatio &&
                JSON.stringify([...t.hashtags].sort()) === JSON.stringify([...currentSettings.hashtags].sort()) &&
                JSON.stringify(t.textOverlays.map(o => o.text)) === JSON.stringify(currentSettings.textOverlays.map(o => o.text));
            return nameMatches && settingsMatch;
        });

        if (exactMatch) {
            console.log('🔍 QuickMode: Found exact match already existing:', exactMatch.id);
            return exactMatch;
        }

        // If name matches the current template, we UPDATE it.
        // If name is different, we CREATE a new one.
        if (currentSettings.name === templateToSave.name) {
            console.log('💾 QuickMode: Updating template', templateToSave.id);
            ...
            console.log('🆕 QuickMode: Creating new template with name', currentSettings.name);
            ...
            // Reload all templates to ensure state is synced
            await slideshowService.loadUserTemplates(userId);
            const updatedTemplates = slideshowService.getSavedTemplates(userId);
            setTemplates(updatedTemplates);
            window.dispatchEvent(new CustomEvent('templatesUpdated'));
            
            return newlySavedTemplate;
        }
    };
    ...
    const handleStartQuickMode = async () => {
        ...
        try {
            ...
            // 1. Save/Update template first (this handles duplication and setting persistence)
            const templateToUse = await saveCurrentSettings();
            if (!templateToUse) throw new Error('Failed to prepare template');
            setSelectedTemplateId(templateToUse.id);

            // 2. Gather images from folder
            ...
            // 3. Create bulk slideshows using the confirmed template
                                        ...
                                        <div
                                            ...
                                            onClick={async () => {
                                                if (selectedTemplateId && selectedTemplateId !== template.id) {
                                                    await saveCurrentSettings(selectedTemplateId);
                                                }
                                                setSelectedTemplateId(template.id);
                                            }}
                                            className={cn(
                                                "p-3 cursor-pointer transition-all rounded-none border-none group relative",
                                        ...
                                        >
                                            <div className="flex justify-between items-start">
                                                ...
                                                <button
                                                    ...
                                                    className="opacity-0 group-hover:opacity-100 p-1.5 hover:bg-white/10 text-white/40 hover:text-red-400 transition-all rounded-none"
                                                >
