        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 xl:p-6">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
            ...
            <div className="lg:col-span-7 space-y-4 flex flex-col">
              ...
              {/* Queue Status — Grouped by Account */}
              {jobQueue.length > 0 && (
                <div className="bg-blue-500/10 rounded-xl p-3 mb-3">
                  ...
                  <div className="space-y-1 max-h-40 overflow-y-auto custom-scrollbar pr-2">
                    {(() => {
                      ...
                      let globalIdx = 0;
                      return Array.from(groups.entries()).map(([accountId, accountJobs], gIdx) => (
                        <div key={accountId}>
                          <div className="flex items-center gap-2 text-[10px] text-blue-300/80 uppercase font-bold mt-2 mb-1 first:mt-0">
                            <User className="w-3 h-3" />
                            <span>{getProfileName(accountId)}</span>
                            <span className="text-muted-foreground/50">— runs {gIdx === 0 ? 'first' : `after account ${gIdx}`}</span>
                            <div className="h-px bg-blue-400/20 flex-1" />
                          </div>
                          {accountJobs.map((job) => {
                            globalIdx++;
                            return (
                              <div key={job.id} className="flex items-center justify-between text-xs bg-black/20 p-2 rounded mb-1">
                                <div className="flex items-center space-x-3">
                                  <span className="font-mono text-muted-foreground">#{globalIdx}</span>
                                  <div className="flex flex-col">
                                    <span className="font-medium text-white/90">
                                      {job.payload.slideshows.length} Posts • {job.payload.strategy === 'batch' ? 'Batch' : 'Interval'}
                                    </span>
                                    {job.total_batches > 1 && (
                                      <span className="text-primary/80 text-[10px]">Batch {job.batch_index}/{job.total_batches}</span>
                                    )}
                                  </div>
                                </div>
                                <div className="flex items-center space-x-2">
                                  <span className="text-muted-foreground text-[10px]">
                                    {new Date(job.scheduled_start_time).toLocaleString([], { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                                  </span>
                                  <span className={cn(
                                    "px-1.5 py-0.5 rounded-full uppercase text-[10px] font-bold",
              ...
              {/* Sequential Account Order Info */}
              {selectedProfiles.length > 1 && (
                <div className="p-3 bg-primary/5 rounded-xl">
                  <div className="flex items-start gap-2">
                    <Layers className="w-4 h-4 text-primary mt-0.5 flex-shrink-0" />
                    <div>
                      <p className="text-xs font-medium text-primary/90">Sequential Account Scheduling</p>
                      <p className="text-[10px] text-muted-foreground mt-1">
                        Posts will complete on each account before moving to the next:
                      </p>
                      <div className="flex flex-wrap gap-1.5 mt-1.5">
                        {selectedProfiles.map((pId, idx) => (
                          <span key={pId} className="text-[10px] px-2 py-0.5 rounded-full bg-white/10 text-white/80 flex items-center gap-1">
                            <span className="w-3.5 h-3.5 rounded-full bg-primary/30 text-primary text-[9px] font-bold flex items-center justify-center">{idx + 1}</span>
                            {getProfileName(pId)}
                          </span>
                        ))}
                      </div>
