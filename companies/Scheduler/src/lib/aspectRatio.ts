...
/**
 * Compress and resize image for optimized storage and egress
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        // Always compress to JPEG for smallest file size unless it's a small PNG
          ...
          (blob) => {
            if (blob) {
              console.log('📉 Image compressed:', {
                ...
                compressedSize: (blob.size / 1024).toFixed(1) + 'KB',
                ratio: (blob.size / imageFile.size).toFixed(2)
              });
              resolve(blob);
            } else {
