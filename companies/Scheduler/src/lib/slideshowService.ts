...
import { supabaseStorage } from './supabaseStorage'; export class SlideshowService {
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = async () => {
        try {
          ...
          if (ratio > 0) {
            // Set canvas size based on target aspect ratio
            // Default to 1080px width for standard quality
            canvas.width = 1080;
            canvas.height = Math.round(1080 / ratio);

            // "Cover" behavior - ensures image fills the frame even if ratios don't match perfectly
            const imgRatio = img.naturalWidth / img.naturalHeight;
            let drawWidth, drawHeight, offsetX, offsetY;

            if (imgRatio > ratio) {
              // Image is wider than canvas ratio (Landscape)
              drawHeight = canvas.height;
              drawWidth = canvas.height * imgRatio;
              offsetX = -(drawWidth - canvas.width) / 2;
              offsetY = 0;
            } else {
              // Image is taller than canvas ratio (Portrait)
  ...
  private async drawTextOverlay(
    ...
    // The font sizes in our database are optimized for the preview scale (0.4 factor).
    // To match the preview (0.4 on 420px) to the canvas (1080px), the theoretical factor is 1.028.
    // User previously reported 1.0 was "massive", but 0.7 was "too small". 
    // We'll use 1.0 for true parity and ensure auto-wrapping and centering are perfect.
    const generationScaleFactor = 1.0;
    const scaledFontSize = (fontSize || 24) * generationScaleFactor;
    const scaledOutlineWidth = (outlineWidth || 2) * generationScaleFactor;
    const scaledGlowIntensity = (glowIntensity || 5) * generationScaleFactor;
    ...
    const canvasFontFamily = fontLoader.getCanvasFontFamily(fontFamily || 'TikTok Sans', effectiveFontWeight);
    ...
    // Split text into lines and handle auto-wrapping
    const maxWidth = (overlay.width || 90) / 100 * canvasWidth;
    const initialLines = text.split('\n');
    const lines: string[] = [];
    
    // Auto-wrap each initial line based on canvas measurement
    ...
    // Calculate starting Y position for vertical alignment (middle anchor)
    // To match TikTokPreview.tsx, the (x, y) coordinates represent the center of the block
    const startY = posY - (lines.length * lineHeight) / 2;
    ...
    lines.forEach((line, lineIndex) => {
      ...
      if (outline) {
        ...
        // CRITICAL FIX: Use round joins and caps for smooth, clean text outlines.
        // Without these, the default 'miter' lineJoin creates sharp spikes at
        // glyph segment intersections, producing ragged/jagged outline edges.
        ctx.lineJoin = 'round';
        ctx.lineCap = 'round';
        ctx.miterLimit = 2;
      ...
      img.onload = async () => {
        try {
          ...
          if (ratio > 0) {
            ...
            // "Cover" behavior - ensures image fills the frame
            const imgRatio = img.naturalWidth / img.naturalHeight;
            let drawWidth, drawHeight, offsetX, offsetY;
