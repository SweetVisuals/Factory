...
Deno.serve(async (req) => {
    ...
    try {
        ...
        // 1. Fetch pending jobs ready to run — ACCOUNT-SEQUENTIAL
        // Only process jobs from one account at a time.
        // If any account has a 'processing' job, only that account's jobs are eligible.
        // Otherwise, pick the account with the earliest scheduled batch.
        const now = new Date().toISOString()

        // Check if any account currently has a processing job
        ...
        if (processingJobs && processingJobs.length > 0) {
            // An account is mid-processing — only pick jobs from that account
            accountFilter = processingJobs[0].account_id;
            console.log(`Account ${accountFilter} has processing jobs — prioritizing`);
        } else {
            // No active processing — find the account with the earliest pending job
            const { data: earliestPending } = await supabase
                .from('job_queue')
                .select('account_id')
                .eq('status', 'pending')
            ...
            if (earliestPending && earliestPending.length > 0) {
                accountFilter = earliestPending[0].account_id;
                console.log(`No active processing — starting account ${accountFilter} (earliest pending)`);
            }
        }

        // Now fetch jobs for that specific account only
        let query = supabase
            .from('job_queue')
            .select('*')
            .eq('status', 'pending')
        ...
        if (!jobs || jobs.length === 0) {
            return new Response(
                JSON.stringify({ message: 'No pending jobs ready to process', accountFilter }),
