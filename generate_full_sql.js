...
let sql = '-- Relay Data Migration\n';

// Campaigns
if (data.campaigns.length > 0) {
    sql += 'INSERT INTO public.campaigns (id, name, user_id, status, pitch, niche, schedule, created_at) VALUES ';
...
for (let i = 0; i < data.leads.length; i += 200) {
    ...
    sql += 'INSERT INTO public.leads (id, user_id, email, name, company, title, phone, linkedin, industry, location, employees, company_news, personalized_email, summary, website, facebook, twitter, instagram, created_at) VALUES ';
...
for (let i = 0; i < data.campaign_leads.length; i += 500) {
    ...
    sql += 'INSERT INTO public.campaign_leads (campaign_id, lead_id) VALUES ';
...
if (data.email_accounts.length > 0) {
    sql += 'INSERT INTO public.email_accounts (id, user_id, email, name, signature, imap_host, imap_port, smtp_host, smtp_port, encrypted_password, warmup_enabled, warmup_filter_tag, warmup_increase_per_day, warmup_daily_limit, warmup_start_date, warmup_status, created_at) VALUES ';
...
for (let i = 0; i < data.email_warmup_progress.length; i += 500) {
    ...
    sql += 'INSERT INTO public.email_warmup_progress (id, email_account_id, date, emails_sent, emails_received, created_at) VALUES ';
...
fs.writeFileSync('migration_full.sql', sql);
console.log('Generated migration_full.sql');
