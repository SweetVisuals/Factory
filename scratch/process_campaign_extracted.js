"use strict";
import { createClient as Ne } from "jsr:@supabase/supabase-js@2";
import De from "npm:nodemailer@6.9.13";
const Te =
    Deno.env.get("DEEPSEEK_API_KEY") || "sk-6733c8ac2b83402b8626e5e253824488",
  Pe = "https://api.deepseek.com",
  Ae =
    Deno.env.get("SUPABASE_URL") || "https://fzcrjogrnujrfxafxbkh.supabase.co",
  Re = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") || "",
  ue = [
  ...
  try {
    ...
    if (_e?.value?.status === "paused")
      return (
        ...
        new Response(JSON.stringify({ message: "Engine paused" }), {
    ...
    const L = (q || []).filter((e) => {
      ...
      return !m || m.status !== "active"
        ? (console.log(
          ...
          !1)
        : o?.business_id === "0269fe06-4607-4c58-9263-12a3930a1dc3" ||
            (m?.name && m.name.toLowerCase().includes("mrmedic")) ||
            (o?.name && o.name.toLowerCase().includes("mrmedic"))
          ? (console.log(
              `Skipping schedule ${e.id} because MrMedic campaigns are disabled.`,
            ),
            !1)
          : !0;
    });
    if (L.length === 0)
      return new Response(
        JSON.stringify({
          message: "No active schedules (all filtered or inactive)",
        }),
        { headers: { "Content-Type": "application/json" } },
      );
    const z = [],
    ...
    for (const e of L) {
      ...
      const u = M.filter((t) => {
        const a = t.email_accounts?.email?.toLowerCase() || "";
        return a.includes("mrmedic") || a.includes("mrmedicevents.co.uk")
          ? (console.log(`Filtering out MrMedic email account: ${a}`), !1)
          : !0;
      });
      if (u.length === 0) {
        console.log(`Schedule ${e.id}: No active non-MrMedic email accounts.`);
        continue;
      }
...
<span style="font-size: 10px; color: #999;">To opt out of future emails, please <a href="${je}" style="color: #999; text-decoration: underline;">click here</a>.</span>`;
    ...
    return new Response(JSON.stringify({ success: !0, processed: z }), {
      headers: { "Content-Type": "application/json" },
    });
  } catch (n) {
    return new Response(String(n?.message ?? n), { status: 500 });
  }
});
